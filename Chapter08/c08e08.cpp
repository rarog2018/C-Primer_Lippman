...
int main(int argc, char *argv[])
{
    ...
    ifstream ifs(argv[1]);
    // construct an ofstream object that'll write to the given file, but this
    // time it is set to an append mode
    ofstream ofs("result.txt", ofstream::app);
    if (ifs.is_open())
    {
        ...
        {                     // read the first transaction
            ...
            {                                     // read the remaining transactions
                ...
                {
                    print(ofs, total) << endl; // print the results
                    total = trans;             // process the next book
                }
            }
            print(ofs, total) << endl; // print the last transaction
        }
        else
        {                                // there was no input

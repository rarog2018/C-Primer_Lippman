...
public:
    ...
    // ADDED: assignment operator that takes an initializer list
...
int main(void)
{
    ...
    // testing the new assignment operator
    cout << "Before assignment: " << endl;
    print(svec);

    cout << endl;
    svec = {"Trylogia Sienkiewicza", "The Black Company", "Elantris"};
...
{
    auto left = lhs.begin(), right = rhs.begin();
    for (; left != lhs.end() && right != lhs.end(); ++left, ++right)
    {
        if (*left < *right)
        {
            return true;
...
// delegates the job to the operator< and operator==
bool operator<=(const StrVec &lhs, const StrVec &rhs)
{
...
bool operator==(const StrVec &lhs, const StrVec &rhs)
{
    // if the sizes match there is a chance that StrVecs are equal
    if (lhs.size() == rhs.size())
    {
        // check every string inside each vector
        return equal(lhs.begin(), lhs.end(), rhs.begin());
        // using equal algorithm, because i use the algorithms too rarely
    }
    else
    {
        return false;

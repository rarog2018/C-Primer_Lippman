...
int main(void)
{
    ...
    cout << "s4 < s1: " << ((s4 < s1) ? "true" : "false") << endl;
...
bool operator<(const String &lhs, const String &rhs)
{
    ...
    // we proceeded the shorter string and now if the lhs was the shorter string
    // then itl will be equal to lhs.end(), so now we want to know if the rhs
    // does not have the same size, if not then all the characters up this point
    // were the same, but lhs is shorter, so lhs < rhs is true
    // alt
    // alternative

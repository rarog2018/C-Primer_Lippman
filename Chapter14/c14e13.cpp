...
// this class should for sure implement + and += operators
// i see reasons for implementing - operator, but i do not see a good reason
// to implement -= operator
// similar story with * and / operators, where they make sense if we use them
// as Sales_data * integer or Sales_data / integer, which can indicate that we
// sold x times more or less items than in this particular Sales_data object
...
{
    lhs.units_sold += rhs.units_sold; // add the members of rhs into
    lhs.revenue += rhs.revenue;       // the members of lhs object
    return lhs;                       // return the object on which the function was called
}

...
{
    // terminate the program if there is not enough arguments
    if (argc != 2)
    {
    ...
    string line;
    ifstream is(argv[1]);
    // terminate the program if the file wasn't successfully open
    if (!is)
    {
        cerr << "Could not open the file" << endl;
    }

    StrBlob obj; // use default constructor
    ...
    // use StrBlobPtr to read the data inside the vector
    // this loop correctly throws exceptions
    /*
    for (StrBlobPtr iter = obj.begin();; iter.incr())
    {
        cout << iter.deref() << endl;
    }
    */

    // this loop prints contents of the vector without errors
    StrBlobPtr iter = obj.begin();
    for (size_t i = 0; i != obj.size(); ++i, iter.incr())
    {
        cout << iter.deref() << endl;
    }

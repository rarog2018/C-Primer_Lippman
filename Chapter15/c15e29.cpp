...
int main(void)
{
    // The discrepency between the total price in this exercise and the exercise
    // e28 happens, because in the previous exercise the vector was holding
    // objects of type Quote which do not have members that manage the discount
    // while this time the vector holds pointers to Quote, under which can hide
    // an object of type Bulk_quote, which has members that manage the discount

    // So this time the discount is properly applied.

    vector<shared_ptr<Quote>> basket{make_shared<Bulk_quote>("98777-X-200", 40, 10, 0.1), make_shared<Bulk_quote>("87666-Y-100", 20, 10, 0.2)};
